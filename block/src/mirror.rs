...
impl AsyncDiskFile for MirroringDiskFile {
    ...
    fn create_async_io(&self, ring_depth: u32) -> BlockResult<Box<dyn AsyncIo>> {
        ...
        let destination_io = self.destination.create_async_io(ring_depth)?;
        Ok(Box::new(MirroringAsyncIo {
            source: source_io,
            destination: destination_io,
...
impl CopyWorker {
    ...
    pub fn spawn(self) -> io::Result<CopyWorkerHandle> {
        ...
        let join = thread::Builder::new()
            .name("block-mirror-copy-worker".into())
            .spawn(move || {
                let mut worker = self;
    ...
    /// then transitions the migration phase to [`MirrorPhase::Synced`].
    fn run(&mut self) -> io::Result<()> {
        ...
        self.state.transition_to_phase(MirrorPhase::Synced);
        Ok(())
    }
