...
impl VirtioDevice for Net {
    ...
    fn activate(
        ...
        let ctrl_thread_count = if has_ctrl_queue { 1 } else { 0 };
        let queue_pair_thread_count = (num_queues - ctrl_thread_count) / 2;
        // Pause waits for the main VMM thread, each spawned RX/TX queue pair
        // thread, and the control queue thread if one is active.
        let pause_barrier_parties = 1 + queue_pair_thread_count + ctrl_thread_count;

        // Size the pause barrier from the epoll threads we are about to spawn,
        // plus the main VMM thread initiating the pause.
