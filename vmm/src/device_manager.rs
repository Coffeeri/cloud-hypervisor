...
impl DeviceManager {
    ...
    /// Returns an iterator over the active mirrors in the block devices.
    /// Each item is a tuple of the block device ID and its `MirrorStatus`.
    pub fn active_block_mirrors(&self) -> impl Iterator<Item = (String, MirrorStatus)> {
        self.block_devices.iter().filter_map(|dev| {
            let disk = dev.lock().unwrap();
            disk.mirror_status().map(|status| (disk.id(), status))
        })
    }
