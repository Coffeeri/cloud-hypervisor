...
impl DeviceManager {
    ...
    ///
    /// Returns an error if `dest_path` already exists, no disk with the
    /// given identifier is attached to the VM, or the destination
    /// cannot be created or opened.
    pub fn mirror_disk(&self, device_id: &str, dest_path: &Path) -> DeviceManagerResult<()> {
        if dest_path.exists() {
            return Err(DeviceManagerError::BlockMirrorDestAlreadyExists(
                device_id.to_string(),
